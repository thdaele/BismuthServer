...
import carpet.bismuth.utils.Messenger;
    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException
    {
        ...
        try
        {
            ...
            if ("setDefault".equalsIgnoreCase(args[0]))
            {
                ...
                if (success)
                {
                    notifyCommandListener(sender, this, args[1] + " will default to: " + args[2]);
                }
                else
                {
            ...
            if ("removeDefault".equalsIgnoreCase(args[0]))
            {
                ...
                if (success)
                {
                    notifyCommandListener(sender, this, args[1] + " will not be set upon restart");
                }
                else
                {
                    throw new WrongUsageException("Unknown setting");
            ...
            if (args.length == 2)
            {
                ...
                msg(sender, Messenger.m(null, "w " + args[0] + ": " + CarpetSettings.get(args[0]) + ", ", "c [change permanently?]", "^w Click to keep the settings in carpet.conf to save across restarts", "?/carpet setDefault " + args[0] + " " + CarpetSettings.get(args[0])));
                return;
            }
            if (sender instanceof EntityPlayer)
            {
                ...
                {
                    options.add(Messenger.m(null, String.format("%s%s %s", (o.equals(CarpetSettings.getDefault(args[0]))) ? "u" : "", (o.equals(CarpetSettings.get(args[0]))) ? "bl" : "y", o), "^g switch to " + o, String.format("?/carpet %s %s", args[0], o)));
            ...
            {
                notifyCommandListener(sender, this, args[0] + " is set to: " + CarpetSettings.get(args[0]));
            }
