...
public class InventoryOptimizer {
	...
	public InventoryOptimizer(InventoryListOptimized stackList, IInventory inventory) {
		...
		if (HopperSettings.debugOptimizedInventories && this.itemRestrictions && !(this.sidedInventory instanceof TileEntityShulkerBox))
	...
	private static long hash(ItemStack stack) {
		...
		hash ^= HashCommon.mix((long) stack.getItemDamage());
		hash ^= HashCommon.mix((long) Objects.hashCode(stack.getTagCompound()));
	...
	private void consistencyCheck() {
		...
		try {
			...
			initialized = false;
			ITextComponent text = new TextComponentString("Detected broken optimizer ( " + e.getMessage() + ") at " + Arrays.toString(e.getStackTrace()));
			MCServer.server.getPlayerList().sendMessage(text);
		}
	}
	...
	public void onItemStackCountChanged(int index, int countChange) {
		...
		if (index >= totalSlots) {
			if (HopperSettings.debugOptimizedInventories) {
				ITextComponent text = new TextComponentString("Detected too large index in InventoryOptimizer.onItemStackCountChanged");
				MCServer.server.getPlayerList().sendMessage(text);
			}
			return;
		}
		...
		if (!wasEmpty || !isEmpty) update(index, prevStack);
			...
			inventoryChanges++;
			if (HopperSettings.debugOptimizedInventories) consistencyCheck();
		}
	}
	...
	public int getMinExtractableItemStackSize(InventoryOptimizer pulledFrom) {
		...
		if (HopperSettings.debugOptimizedInventories && pulledFrom != this)
