package si.bismuth.hopperoptimisations;
...
public class HopperSettings {
	...
	//@Rule(desc = "Optimized Inventory accesses - bloomfilters, cached BlockEntities and improved item transfers", category = {OPTIMIZATION, "hopperoptimizations"}, validate = OptimizedInventoriesRule.class)
	public static boolean optimizedInventories = false;

	//@Rule(desc = "Simplified hopper box shape when picking up items. This box contains the ring around the hopper's bowl.", category = {OPTIMIZATION, FEATURE, "hopperoptimizations"}, validate = EntityHopperInteraction.class)
	...
	//@Rule(desc = "Reworked interaction between hoppers and entities. Entities look for hoppers instead of hoppers searching for entities.", category = {OPTIMIZATION, EXPERIMENTAL, "hopperoptimizations"}, validate = EntityHopperInteraction.class)
	public static boolean optimizedEntityHopperInteraction = false;

	//@Rule(desc = "Removes check whether item entities have to move out of another entity (boat, shulker).", category = {OPTIMIZATION, FEATURE, "hopperoptimizations"})
	public static boolean simplifiedItemElevatorCheck = false;

	//@Rule(desc = "Speeds up checking whether an itemStack is empty by using cached information from vanilla.", category = {OPTIMIZATION, "hopperoptimizations"})
	public static boolean optimizedItemStackEmptyCheck = false;

	//@Rule(desc = "Doesn't do comparator updates when an item transfer fails unlike vanilla (when the transfer would have changed the signal strength)", category = {OPTIMIZATION, FEATURE, "hopperoptimizations"})
	...
	//@Rule(desc = "Make hoppers only check for newly created blockentities when receiving a block update. Requires optimizedEntityHopperInteraction.", category = {OPTIMIZATION, "hopperoptimizations"}, validate = EntityHopperInteraction.class)
	//Using the EntityHopperInteraction counter to invalidate entity caches on pickup box change.
